...
def popular_banco():
    ...
    def tabela_tem_dados(tabela):
        """Verifica se a tabela já possui registros"""
        registros = repo.listar(tabela)
        return len(registros) > 0

    if not tabela_tem_dados("clientes"):
        clientes = [
    ...
    if not tabela_tem_dados("carros"):
        carros = [
    ...
    if not tabela_tem_dados("agencias"):
        agencias = [
    ...
    if not tabela_tem_dados("funcionarios"):
        funcionarios = [
    ...
    if not tabela_tem_dados("contratos"):
        contratos = [
        ...
        for contrato in contratos:
            repo.adicionar("contratos", contrato.to_dict())
    
    if not tabela_tem_dados("manutencoes"):
        manutencoes = [
        ...
        for manutencao in manutencoes:
            repo.adicionar("manutencoes", manutencao.to_dict())
    
    print("Banco de dados populado com sucesso (se necessário).")
